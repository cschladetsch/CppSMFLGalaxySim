...
void ParticleGalaxyMode::CreateGalaxyPreset(int preset) {
    ...
    switch (preset) {
        case 0: { // Milky Way-like Spiral Galaxy
            ...
            for (int i = 0; i < 8000; ++i) {
                ...
                float bulgeHeight = std::normal_distribution<float>(0.0f, coreRadius * 0.3f)(rng_);
                
                Graphics::Particle particle;
                particle.position = center + glm::vec2(
                    r * std::cos(angle),
                    r * std::sin(angle) + bulgeHeight
                );
                
                // Orbital velocity
                float orbitalSpeed = std::sqrt(gravitationalConstant_ * blackHole.mass / r) * speedDist(rng_);
                ...
                particle.color.r = std::min(255, static_cast<int>(particle.color.r * brightnessFactor));
                particle.color.g = std::min(255, static_cast<int>(particle.color.g * brightnessFactor));
                particle.color.b = std::min(255, static_cast<int>(particle.color.b * brightnessFactor));
            ...
            for (int i = 0; i < 20000; ++i) {
                float radius = coreRadius + (maxRadius - coreRadius) * std::pow(radiusDist(rng_), 0.6f);
                ...
                if (radius < barRadius) {
                    ...
                    // Spiral arms
                    spiralAngle = armBaseAngle + std::log(radius / barRadius) * windingFactor;
                }
                ...
                if (inArm < 0.6f) {
                    ...
                    armSpread = std::uniform_real_distribution<float>(-armAngleOffset/2, armAngleOffset/2)(rng_);
                }
                
                spiralAngle += armSpread;
                
                // Disk height decreases with radius
                float heightScale = 1.0f - (radius - coreRadius) / (maxRadius - coreRadius) * 0.7f;
                ...
                float orbitalSpeed = std::sqrt(gravitationalConstant_ * blackHole.mass / radius) * speedDist(rng_);
                glm::vec2 toCenter = glm::normalize(center - particle.position);
                particle.velocity = glm::vec2(-toCenter.y, toCenter.x) * orbitalSpeed;
                ...
                if (distanceRatio > 0.7f && radiusDist(rng_) > (1.0f - distanceRatio) * 2.0f) {
            ...
            for (int c = 0; c < numClusters; ++c) {
                float clusterRadius = maxRadius * std::uniform_real_distribution<float>(0.3f, 1.2f)(rng_);
                float clusterAngle = angleDist(rng_);
                float clusterHeight = std::normal_distribution<float>(0.0f, 100.0f)(rng_);
                
                glm::vec2 clusterCenter = center + glm::vec2(
                    clusterRadius * std::cos(clusterAngle),
                ...
                for (int s = 0; s < starsInCluster; ++s) {
                    ...
                    float orbitalSpeed = std::sqrt(gravitationalConstant_ * blackHole.mass / clusterRadius) * 0.8f;
            ...
            spdlog::info("Created Milky Way galaxy with {} stars", galaxyParticles_.size());
            break;
        }
        ...
        case 1: { // Binary Star System
            ...
            for (int i = 0; i < 30000; ++i) {
                ...
                Graphics::Particle particle;
                particle.position = starPos + glm::vec2(
                    radius * std::cos(angle),
                    radius * std::sin(angle) * 0.3f // Flatten the disk
                );
                
                // Orbital velocity
                float orbitalSpeed = std::sqrt(gravitationalConstant_ * massiveObjects_[starIndex].mass / radius);
                glm::vec2 toStar = glm::normalize(starPos - particle.position);
                particle.velocity = massiveObjects_[starIndex].velocity + glm::vec2(-toStar.y, toStar.x) * orbitalSpeed;
                
                particle.color = (starIndex == 0) ? 
                    sf::Color(255, 220, 180, 150) : 
        ...
        case 2: { // Globular Cluster
            ...
            for (int i = 0; i < 40000; ++i) {
                ...
                float r = clusterRadius * std::pow(radiusDist(rng_), 0.333f); // Cube root for uniform volume
    ...
    spdlog::info("Created galaxy preset {} with {} particles and {} massive objects", 
...
void ParticleGalaxyMode::UpdatePhysics(float deltaTime) {
    ...
    for (std::size_t i = 0; i < massiveObjects_.size(); ++i) {
        ...
        for (std::size_t j = 0; j < massiveObjects_.size(); ++j) {
            ...
            totalForce += CalculateGravitationalForce(
                massiveObjects_[i].position,
                massiveObjects_[j].position,
                massiveObjects_[i].mass,
                massiveObjects_[j].mass
            );
        }
    ...
    const std::size_t batchSize = galaxyParticles_.size() / threadPool_->GetNumThreads();
    std::vector<std::future<void>> futures;
    
    for (std::size_t i = 0; i < threadPool_->GetNumThreads(); ++i) {
        std::size_t start = i * batchSize;
        std::size_t end = (i == threadPool_->GetNumThreads() - 1) ? galaxyParticles_.size() : (i + 1) * batchSize;
...
void ParticleGalaxyMode::UpdateParticlePhysics(std::size_t start, std::size_t end, float deltaTime) {
    for (std::size_t i = start; i < end && i < galaxyParticles_.size(); ++i) {
        if (!galaxyParticles_[i].active) continue;
        ...
        for (const auto& massiveObject : massiveObjects_) {
            totalForce += CalculateGravitationalForce(
                galaxyParticles_[i].position,
                massiveObject.position,
                galaxyParticles_[i].mass,
                massiveObject.mass
            );
        }
        ...
        auto windowSize = GetDisplaySystem().GetWindow().getSize();
        glm::vec2 center(windowSize.x * 0.5f, windowSize.y * 0.5f);
        float distanceFromCenter = glm::length(galaxyParticles_[i].position - center);
...
glm::vec2 ParticleGalaxyMode::CalculateGravitationalForce(const glm::vec2& pos1, const glm::vec2& pos2, 
...
void ParticleGalaxyMode::Render(sf::RenderTarget& target) {
    ...
    if (showGrid_) {
        ...
        for (float x = 0; x < windowSize.x; x += gridSpacing) {
            renderer.DrawLine(glm::vec2(x, 0), glm::vec2(x, windowSize.y), gridColor, 0.5f);
        }
        for (float y = 0; y < windowSize.y; y += gridSpacing) {
            renderer.DrawLine(glm::vec2(0, y), glm::vec2(windowSize.x, y), gridColor, 0.5f);
        }
    }
    ...
    for (const auto& body : massiveObjects_) {
        ...
        for (int i = 1; i <= 3; ++i) {
            ...
            renderer.DrawCircle(body.position, body.radius + i * 5.0f, glowColor, false);
        }
    }
    ...
    if (font.loadFromFile("Assets/Fonts/arial.ttf")) {
        ...
        info += "Particles: " + std::to_string(particleSystem_->GetActiveParticleCount()) + "\n";
        info += "Time Dilation: " + std::to_string(timeDilation_) + "x\n";
        info += "Preset: " + std::to_string(currentPreset_ + 1) + "/" + std::to_string(NUM_PRESETS) + "\n";
