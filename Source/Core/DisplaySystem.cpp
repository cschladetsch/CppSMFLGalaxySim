...
bool DisplaySystem::Initialize(const DisplayConfig& config) {
    ...
    sf::Uint32 style = config.fullscreen ? sf::Style::Fullscreen : sf::Style::Default;
    ...
    inputManager_->RegisterEventHandler(InputEvent::Type::KeyPressed,
    ...
    spdlog::info("Window: {}x{}, Fullscreen: {}, VSync: {}", 
...
void DisplaySystem::Shutdown() {
    if (isRunning_) {
        ...
        if (currentModeIndex_ < visualModes_.size() && visualModes_[currentModeIndex_]) {
        ...
        if (currentModeIndex_ < visualModes_.size() && visualModes_[currentModeIndex_]) {
            ...
            switch (event.type) {
                ...
                case sf::Event::MouseMoved:
                    ...
                    inputEvent.mouseMove.position = glm::vec2(event.mouseMove.x, event.mouseMove.y);
                    visualModes_[currentModeIndex_]->HandleInput(inputEvent);
                    break;
                ...
                case sf::Event::Resized:
                    ...
                    visualModes_[currentModeIndex_]->OnResize(event.size.width, event.size.height);
                    break;
...
void DisplaySystem::UpdatePerformanceMetrics() {
    ...
    auto elapsed = std::chrono::duration_cast<std::chrono::duration<float>>(currentTime - lastFrameTime_).count();
