...
Camera2D::Camera2D()
    : view_(), position_(0.0f, 0.0f), size_(1920.0f, 1080.0f), rotation_(0.0f),
      zoom_(1.0f), needsUpdate_(true) {
  Update();
}

Camera2D::Camera2D(const sf::FloatRect &viewport)
    : view_(), position_(viewport.left + viewport.width * 0.5f,
                         viewport.top + viewport.height * 0.5f),
      size_(viewport.width, viewport.height), rotation_(0.0f), zoom_(1.0f),
      needsUpdate_(true) {
