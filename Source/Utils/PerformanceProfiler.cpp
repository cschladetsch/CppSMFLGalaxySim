...
PerformanceProfiler::PerformanceProfiler()
    : frameTimes_(FRAME_TIME_BUFFER_SIZE, 0.0f)
    , frameTimeIndex_(0)
...
void PerformanceProfiler::EndFrame() {
    ...
    auto frameDuration = std::chrono::duration_cast<std::chrono::duration<float>>(frameEndTime - frameStartTime_).count();
...
void PerformanceProfiler::BeginSection(const std::string& name) {
    ...
    activeSections_[name] = SectionTimer{
...
void PerformanceProfiler::EndSection(const std::string& name) {
    ...
    auto duration = std::chrono::duration_cast<std::chrono::duration<double>>(endTime - it->second.startTime).count();
    
    auto& data = sectionData_[name];
    data.minTime = std::min(data.minTime, duration);
    data.maxTime = std::max(data.maxTime, duration);
    data.averageTime = (data.averageTime * data.sampleCount + duration) / (data.sampleCount + 1);
    data.sampleCount++;
...
void PerformanceProfiler::LogMemoryUsage(std::size_t bytes) {
    ...
    while (expected < bytes && !peakMemoryUsage_.compare_exchange_weak(expected, bytes)) {
...
PerformanceProfiler::ProfileData PerformanceProfiler::GetSectionData(const std::string& name) const {
    std::lock_guard<std::mutex> lock(mutex_);
