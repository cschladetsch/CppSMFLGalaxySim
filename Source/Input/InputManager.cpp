...
InputManager::InputManager() {
    ...
    std::fill(std::begin(mouseButtonStates_), std::end(mouseButtonStates_), false);
}
...
void InputManager::ProcessEvent(const sf::Event& event) {
    ...
    switch (event.type) {
        case sf::Event::KeyPressed:
            ...
            currentModifiers_ = 0;
            if (event.key.alt) currentModifiers_ |= MOD_ALT;
            if (event.key.control) currentModifiers_ |= MOD_CONTROL;
            if (event.key.shift) currentModifiers_ |= MOD_SHIFT;
            if (event.key.system) currentModifiers_ |= MOD_SYSTEM;
        ...
        case sf::Event::MouseButtonPressed:
            ...
            inputEvent.mouseButton.position = glm::vec2(event.mouseButton.x, event.mouseButton.y);
            
            for (const auto& handler : eventHandlers_[InputEvent::Type::MouseButtonPressed]) {
                handler(inputEvent);
            }
            break;
        ...
        case sf::Event::MouseButtonReleased:
            ...
            for (const auto& handler : eventHandlers_[InputEvent::Type::MouseButtonReleased]) {
                handler(inputEvent);
            }
            break;
        ...
        case sf::Event::MouseWheelScrolled:
            ...
            inputEvent.mouseWheel.position = glm::vec2(event.mouseWheelScroll.x, event.mouseWheelScroll.y);
            
            for (const auto& handler : eventHandlers_[InputEvent::Type::MouseWheelScrolled]) {
                handler(inputEvent);
            }
            break;
        ...
        case sf::Event::Resized:
            ...
            for (const auto& handler : eventHandlers_[InputEvent::Type::WindowResized]) {
                handler(inputEvent);
            }
            break;
...
void InputManager::RegisterEventHandler(InputEvent::Type type, EventHandler handler) {
    eventHandlers_[type].push_back(handler);
}

void InputManager::RegisterKeyBinding(const std::string& action, sf::Keyboard::Key key, unsigned int modifiers) {
