...
public:
    ...
    using KeyBinding = std::pair<sf::Keyboard::Key, unsigned int>; // Key + modifiers
    ...
    void RegisterKeyBinding(const std::string& action, sf::Keyboard::Key key, unsigned int modifiers = 0);
...
private:
    std::unordered_map<InputEvent::Type, std::vector<EventHandler>> eventHandlers_;
